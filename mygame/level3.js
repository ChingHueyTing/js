class level3 extends Phaser.Scene {
  ...
  create() {
    ...
    this.scoreText.setScrollFactor(0);
     ...
     // Define a shooting function
     this.shoot = () => {
      // Create a bullet sprite at the player's position
      const bullet = this.physics.add.sprite(this.player.x, this.player.y, 'bulletimg');

      // Set velocity for the bullet to move it in the desired direction
      bullet.setVelocityX(300); // Adjust velocity as needed

      // Play shooting animation or sound if desired

      this.physics.add.collider(bullet, this.enemyGroup, (bullet, enemy) => {
  ...
  // Bind shooting function to the Enter key press event
...
this.tweens.add({
 ...
 this.heart3.setDepth(1); // Set the depth to 1 (or higher if needed) to render it on top
   ...
   this.physics.add.overlap(this.player,this.enemy9,this.hitEnemy9,null,this);
   this.physics.add.overlap(this.player,this.enemy10,this.hitEnemy10,null,this);
  ...
  this.heart3.setPosition(heart3X, heart3Y);  
    ...
    // In update()
    if (this.player.x > 1472 && this.player.y > 1078 && this.player.y < 1150) {
        console.log("Door3");
        this.room3();
      }
...
collectYellow2(player, item) {
  console.log("player collect yellow2")
  item.disableBody(true, true)
  this.score += 1;
  this.scoreText.setText('Energy Drink:' + this.score);
  return false
...
collectYellow3(player, item) {
  console.log("player collect yellow3")
  item.disableBody(true, true)
  this.score += 1;
  this.scoreText.setText('Energy Drink:' + this.score);
  return false
